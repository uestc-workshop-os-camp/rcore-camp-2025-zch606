...
impl TaskManager {
    ...
    pub fn get_current_syscall_count(&self, syscall_id: usize) -> Option<usize> {
        ...
        if syscall_id < MAX_SYSCALL_NUM {
            Some(inner.tasks[current].task_syscall_count[syscall_id])
        } else {
            None
        }
    }
    
    /// Add the syscall count of current task
    pub fn add_current_syscall_count(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        if syscall_id < MAX_SYSCALL_NUM {
...
/// Add the syscall count of current task
pub fn current_add_syscall_count(syscall_id: usize) {
    TASK_MANAGER.add_current_syscall_count(syscall_id);
}
/// Get the syscall count of current task
pub fn current_get_syscall_count(syscall_id: usize) -> Option<usize> {
